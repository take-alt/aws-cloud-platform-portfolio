...
### 1. 「運用経験者らしい」ポートフォリオにした
監視・通知・運用手順まで書き切ることで、単なる学習用の IaC リポジトリではなく、
**現場運用を理解したクラウドエンジニア候補**として見てもらえる構成を意識しました。

### 2. 構築だけで終わらせていない
アーキテクチャ図、監視項目、運用思想、CI/CD まで含めることで、
「作った後にどう守るか」まで示しています。

### 3. 面接で説明しやすい構成にした
以下の流れで説明できるように設計しています。
- どんな課題を解決したいか
- なぜこの技術を選んだか
- 可用性・セキュリティ・運用性をどう担保したか
- 変更をどう安全に適用するか
- 障害時にどう検知し、どう動くか
...
> 監視・運用保守の経験を通じて、手動運用や属人的な障害対応の課題を感じていました。
> そこで本ポートフォリオでは、Terraform による再現性のある AWS 基盤構築、GitHub Actions による品質担保と自動デプロイ、
> CloudWatch を用いた監視・通知までを一貫して実装し、構築だけでなく運用を前提にしたクラウド基盤を表現しました。

---

## 今後の改善案

- Route53 / ACM を追加し HTTPS 化
- WAF の導入
- OIDC による GitHub Actions から AWS へのセキュア接続
- Blue/Green デプロイ
- Systems Manager を用いた運用自動化
- コスト最適化の可視化
...
- 監視・運用保守の実務経験がある
- AWS の設計・構築に踏み込める
- IaC と CI/CD で再現性と安全性を担保できる
- 監視設計や運用設計まで含めて考えられる

資格や学習実績だけでなく、**実務に近いかたちで設計・構築・運用を一気通貫で考えられること**を示すことが、このポートフォリオの目的です。
